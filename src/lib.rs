    ...
    F: FnMut(u8, &u8) -> u8 + Copy,
{
    ...
    checksum_func: Option<&'a F>
}

impl<'a, F> Default for Message<'a, F>
where
    F: FnMut(u8, &u8) -> u8 + Copy,
{
    fn default() -> Message<'a, F> {
        let s = Message {
    ...
    F: FnMut(u8, &u8) -> u8 + Copy,
{
    ...
    pub fn build(mut self, data: &'a[u8], synch: Option<Synchronization>) -> Result<Message<'a, F>, &'static str> {
        if let Some(s) = synch {
            self.synch = s;
        }
        self.data = Some(data);
